...
async def admin_confirm_match(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if not ok:
        if reason == "player_busy":
            msg = texts.ADMIN_PLAYER_BUSY
        else:
            msg = texts.ERROR_GENERIC
        await callback.message.edit_text(msg, reply_markup=keyboards.back_to_admin(), parse_mode="HTML")
        return
    ...
    await callback.message.edit_text(
        texts.ADMIN_MATCH_CREATED.format(
            match_id=match_id,
            p1_flag=p1.team_flag if p1 else "",
